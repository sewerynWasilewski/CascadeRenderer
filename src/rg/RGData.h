...
struct RGPassData {
  const char*   name;
  RGPassFlags   flags;
  RHIQueueType  queue;        // derived from flags at addPass time
...
struct RGResourceData {
  const char*     name;
  RHIResourceKind kind;
  RGResourceType  type;
  RHIMemoryType   memory_type;    // which memory pool this resource belongs to
  ...
  RHIQueueMask    queue_mask;     // which queues access this - accumulated during setup
...
// GPU barrier scheduled before dst_pass. src/dst_pass are global_index values (not pass IDs).
// before/after_usage stored as u32 bitmask (RHIUsage values) to stay POD-friendly.
struct RGBarrier {
  u32            resource_id;
  u32            before_usage; // RHIUsage bitmask
  u32            after_usage;  // RHIUsage bitmask
  u32            src_pass;     // global_index
  u32            dst_pass;     // global_index
  RHIBarrierKind kind;
};
